...
CRYPTOGEN_TOOL_PATH=${PROJECT_PATH}/tools/yzchain-cryptogen
CRYPTOGEN_TOOL_BIN=${CRYPTOGEN_TOOL_PATH}/bin/yzchain-cryptogen
...
function generate_config() {
    ...
    PPROF_PORT=24321
    TRUSTED_PORT=13301
    VM_GO_CONTAINER_NAME_PREFIX="yzchain-vm-go-container"
...
#    for ((i = 1; i < $NODE_CNT + 1; i = i + 1)); do
        ...
        cp $CONFIG_TPL_PATH/yzchain.tpl node$i/yzchain.yml

        xsed "s%{net_port}%$(($P2P_PORT+$i-1))%g" node$i/yzchain.yml
        xsed "s%{rpc_port}%$(($RPC_PORT+$i-1))%g" node$i/yzchain.yml
        xsed "s%{monitor_port}%$(($MONITOR_PORT+$i-1))%g" node$i/yzchain.yml
        xsed "s%{pprof_port}%$(($PPROF_PORT+$i-1))%g" node$i/yzchain.yml
        xsed "s%{trusted_port}%$(($TRUSTED_PORT+$i-1))%g" node$i/yzchain.yml
        xsed "s%{enable_vm_go}%$ENABLE_VM_GO%g" node$i/yzchain.yml
        xsed "s%{dockervm_container_name}%"${VM_GO_CONTAINER_NAME_PREFIX}$i"%g" node$i/yzchain.yml
        xsed "s%{vm_go_runtime_port}%$(($VM_GO_RUNTIME_PORT+$i-1))%g" node$i/yzchain.yml
        xsed "s%{vm_go_engine_port}%$(($VM_GO_ENGINE_PORT+$i-1))%g" node$i/yzchain.yml
        xsed "s%{vm_go_log_level}%$VM_GO_LOG_LEVEL%g" node$i/yzchain.yml
        xsed "s%{vm_go_protocol}%$VM_GO_TRANSPORT_PROTOCOL%g" node$i/yzchain.yml

        system=$(uname)

        if [ "${system}" = "Linux" ]; then
            for ((k = $NODE_CNT; k > 0; k = k - 1)); do
                xsed "/  seeds:/a\    - \"/ip4/127.0.0.1/tcp/$(($P2P_PORT+$k-1))/p2p/{org${k}_peerid}\"" node$i/yzchain.yml
            done
        else
...
" node$i/yzchain.yml
        ...
        for ((j = 1; j < $CHAIN_CNT + 1; j = j + 1)); do
            xsed "s%#\(.*\)- chainId: chain${j}%\1- chainId: chain${j}%g" node$i/yzchain.yml
            xsed "s%#\(.*\)genesis: ../config/{org_path$j}/chainconfig/bc${j}.yml%\1genesis: ../config/{org_path$j}/chainconfig/bc${j}.yml%g" node$i/yzchain.yml
            ...
            do
                ...
                if  [ $j -eq 1 ]; then
                    xsed "s%{org${c}_peerid}%$peerId%g" node$i/yzchain.yml
                fi
                # cp ca
                ...
                if  [ $c -eq $i ]; then
                    if [ $c -gt $NODE_CNT ]; then
                      xsed "s%{node_cert_path}%node\/common1\/common1.sign%g" node$i/yzchain.yml
                      xsed "s%{net_cert_path}%node\/common1\/common1.tls%g" node$i/yzchain.yml
                      xsed "s%{rpc_cert_path}%node\/common1\/common1.tls%g" node$i/yzchain.yml
                    else
                      xsed "s%{node_cert_path}%node\/consensus1\/consensus1.sign%g" node$i/yzchain.yml
                      xsed "s%{net_cert_path}%node\/consensus1\/consensus1.tls%g" node$i/yzchain.yml
                      xsed "s%{rpc_cert_path}%node\/consensus1\/consensus1.tls%g" node$i/yzchain.yml
                    fi
                    xsed "s%{org_path}%$file%g" node$i/chainconfig/bc$j.yml
                    xsed "s%{node_cert_path}%node\/consensus1\/consensus1.sign%g" node$i/yzchain.yml
                    xsed "s%{net_cert_path}%node\/consensus1\/consensus1.tls%g" node$i/yzchain.yml
                    xsed "s%{rpc_cert_path}%node\/consensus1\/consensus1.tls%g" node$i/yzchain.yml
                    xsed "s%{org_id}%$file%g" node$i/yzchain.yml
                    xsed "s%{org_path}%$file%g" node$i/yzchain.yml
                    xsed "s%{org_path$j}%$file%g" node$i/yzchain.yml
