...
alreadyBuild=$3
enableDocker=$4
if [ ! -d "../../tools/yzchain-cryptogen" ]; then
  echo "not found yzchain-go/tools/yzchain-cryptogen"
  echo "  default port is 11391 12391, you can use "
  echo "              cd yzchain-go/tools"
  echo "              ln -s ../../yzchain-cryptogen ."
  echo "              cd yzchain-cryptogen && make"
  exit 0
fi
...
#    echo -e "\nINFO\n\n\n" | ./prepare_pwk.sh $node_count $chain_count
  ...
  echo "【yzchain】 process..."
  ps -ef | grep yzchain
  yzchain_count=$(ps -ef | grep yzchain | wc -l)
  if [ $yzchain_count -lt 4 ]; then
    echo "build error"
    exit
  fi
...
function prepare_cmc() {
  ...
  rm -rf testdata
  mkdir testdata
  cp $PROJECT_PATH/tools/yzc/testdata/sdk_config_pwk.yml testdata/
...
function cmc_test() {
  ...
  ./cmc client contract user create \
    ...
    --admin-org-ids=yz-org1.yzchain.org,yz-org2.yzchain.org,yz-org3.yzchain.org,yz-org4.yzchain.org \
    --admin-key-file-paths=./testdata/crypto-config/yz-org1.yzchain.org/admin/admin.key,./testdata/crypto-config/yz-org2.yzchain.org/admin/admin.key,./testdata/crypto-config/yz-org3.yzchain.org/admin/admin.key,./testdata/crypto-config/yz-org4.yzchain.org/admin/admin.key \
    --sync-result=true \
    --params="{}"
...
function cat_log() {
  sleep 1
  grep --color=auto "all necessary\|ERROR\|put block" $PROJECT_PATH/build/release/yzchain-*1*/log/system.log
}
