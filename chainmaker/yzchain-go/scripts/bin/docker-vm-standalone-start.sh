...
EXPOSE_PORT=22351
RUNTIME_PORT=32351
CONTAINER_NAME=yzchain-vm-go
IMAGE_NAME="yzchainofficial/yzchain-vm-engine:v2.3.2"
...
read -r -p "input container name(default 'yzchain-docker-vm'): " tmp
if  [ -n "$tmp" ] ;then
  CONTAINER_NAME=$tmp
else
  echo "container name use default: 'yzchain-docker-vm'"
fi
