...
config_file="../config/{org_id}/yzchain.yml"
# config_file="../../config/yz-org1-solo/yzchain.yml"
eval $(parse_yaml "$config_file" "yzchain_")

VM_GO_IMAGE_NAME="yzchainofficial/yzchain-vm-engine:v2.3.2"
DOCKER_VM_IMAGE_NAME="yzchainofficial/yzchain-vm-engine:v2.3.1"
START_FULL_MODE=""
...
for i in "$@";do
    case "$i" in
        ...
        *)
            ...
            echo $"  full: start yzchain node with the vm containers"
            echo $"  alone: start yzchain node alone, WITHOUT vm containers"
...
function start_vm_go() {
  ...
  mount_path=$yzchain_vm_go_data_mount_path
  log_path=$yzchain_vm_go_log_mount_path
  ...
  protocol=$yzchain_vm_go_protocol
  vm_go_log_level=$yzchain_vm_go_log_level
  dockervm_config_path=$yzchain_vm_go_dockervm_config_path
  runtime_server_host=$yzchain_vm_go_runtime_server_host
  runtime_server_port=$yzchain_vm_go_runtime_server_port
  contract_engine_port=$yzchain_vm_go_contract_engine_port
  rpc_timeout=$yzchain_vm_go_dial_timeout
  rpc_max_send_size=$yzchain_vm_go_max_send_msg_size
  rpc_max_recv_size=$yzchain_vm_go_max_recv_msg_size
  log_in_console=$yzchain_vm_go_log_in_console
  max_concurrency=$yzchain_vm_go_max_concurrency
  slow_disable=$yzchain_vm_go_slow_disable
  slow_step_time=$yzchain_vm_go_slow_step_time
  slow_tx_time=$yzchain_vm_go_slow_tx_time
  process_timeout=$yzchain_vm_go_process_timeout
...
#  fi
  ...
  echo "start docker vm service container succeed2: $container_name"
...
function start_docker_vm_go() {
  ...
  # concat mount_path and log_path for container to mount
  docker_vm_mount_path=$yzchain_vm_docker_go_dockervm_mount_path
  docker_vm_log_path=$yzchain_vm_docker_go_dockervm_log_path
  docker_vm_log_level=$yzchain_vm_docker_go_log_level
  docker_vm_log_in_console=$yzchain_vm_docker_go_log_in_console
...
pid=$(ps -ef | grep yzchain | grep "\-c ../config/{org_id}/yzchain.yml {tagName}" | grep -v grep |  awk  '{print $2}')
if [ -z "${pid}" ];then
    # check if enable go vm
    if [[ $yzchain_vm_go_enable == "true" ]]; then
     enable_go_vm_container=true
    fi
    # check if enable Deprecated docker vm
    if [[ $yzchain_vm_docker_go_enable_dockervm == "true" && $yzchain_vm_docker_go_uds_open == "true" ]]; then
    ...
    # start yzchain
    #nohup ./yzchain start -c ../config/{org_id}/yzchain.yml {tagName} > /dev/null 2>&1 &
    nohup ./yzchain start -c ../config/{org_id}/yzchain.yml {tagName} > panic.log 2>&1 &
