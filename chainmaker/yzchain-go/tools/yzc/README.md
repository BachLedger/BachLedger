# 命令行工具
<span id="section_yzc"></span>

## 简介
yzc`(YzChain Client)`是YzChain提供的命令行工具，用于和YzChain链进行交互以及生成证书等功能。yzc基于go语言编写，通过使用YzChain的go语言sdk（使用grpc协议）达到和YzChain链进行交互的目的。<br>
yzc的详细日志请查看`./sdk.log`
...
# 编译yzc
$ cd ~/yzchain
$ git clone -b v2.0.0 https://git.qualink.com/yunzhougyhlw/yunzhoulian/yzchain/yzchain-go.git
$ cd yzchain-go/tools/yzc
$ go mod download
$ go build
# 启动测试链
$ cd ~/yzchain/yzchain-go/tools
$ git clone -b master https://git.qualink.com/yunzhougyhlw/yunzhoulian/yzchain/yzchain-cryptogen.git
$ cd yzchain-cryptogen && make
$ cd ~/yzchain/yzchain-go/scripts
$ ./prepare.sh 4 1 # 这里以4组织，每组织1共识节点为例。使用默认参数，一直回车。
$ ./build_release.sh
$ ./cluster_quick_start.sh normal
# 配置测试数据
$ cd ~/yzchain
$ cp -rf ./yzchain-go/build/crypto-config ./yzchain-go/tools/yzc/testdata/ # 使用yzchain-cryptogen生成的测试链的证书
# 配置完成
$ cd ~/yzchain/yzchain-go/tools/yzc
$ ./yzc --help
```
...
yzc 依赖 sdk-go 配置文件。<br>
编译&配置 步骤使用的是 SDK配置模版 <br>
可通过修改 ~/yzchain/yzchain-go/tools/yzc/testdata/sdk_config.yml 实现自定义配置。<br>
比如 `user_key_file_path`,`user_crt_file_path`,`user_sign_key_file_path`,`user_sign_crt_file_path`<br>
这四个参数可设置为普通用户或admin用户的证书/私钥路径。设置后yzc将会以对应用户身份与链建立连接。<br>
其他详细配置项请参看 ~/yzchain/yzchain-go/tools/yzc/testdata/sdk_config.yml 中的注解。<br>
...
#### 私钥管理
  ...
  **示例：**
  ...
  ```sh
  $ ./yzc key gen -a ECC_P256 -n ca.key -p ./
  ```
...
#### 证书管理
  - 生成ca证书
    ...
    **示例**
    ...
    ```sh
    $ ./yzc cert ca -c yz-org1.yzchain.org -k ca.key -H sha256 --ou root-ca -n ca.crt -p ./ --org yz-org1
    ```
  ...
  - 生成crl列表
    ...
    **示例**
    ...
    ```sh
    $ ./yzc cert crl -C ./ca.crt -K ca.key --crl-path=./client1.crl --crt-path=../sdk/testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.crt
    ```
  ...
  - 生成csr文件
    ...
    **示例**
    ...
    ```sh
    $ ./yzc cert csr -c yz-org1.yzchain.org -k ../sdk/testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.key -n client1.csr -p ./ --ou root-ca -o yz-org1
    ```
  ...
  - 颁发证书
    ...
    **示例**
    ...
    ```sh
    $ ./yzc cert issue -C ca.crt -K ca.key -r client1.csr -H sha256 -n client1.crt -p ./
    ```
  ...
  - 根据证书获取节点Id
    ...
    **示例**
    ...
    ```sh
    $ ./yzc cert nid --node-cert-path=../sdk/testdata/crypto-config/yz-org1.yzchain.org/node/consensus1/consensus1.tls.crt
...
#### 交易功能
##### 用户合约
  yzc的交易功能用来发送交易和链进行交互，主要参数说明如下：

  ```sh
    sdk配置文件flag
    --sdk-conf-path：指定yzc使用sdk的配置文件路径
    ...
    --admin-crt-file-paths: admin签名者的tls crt文件的路径列表. 单签模式下只需要填写一个即可, 离线多签模式下多个需要用逗号分割
        比如 ./yz-org1.yzchain.org/admin1.tls.crt,./yz-org2.yzchain.org/admin1.tls.crt
    --admin-key-file-paths: admin签名者的tls key文件的路径列表. 单签模式下只需要填写一个即可, 离线多签模式下多个需要用逗号分割
        比如 ./yz-org1.yzchain.org/admin1.tls.key,./yz-org2.yzchain.org/admin1.tls.key
    ...
    --method：指定调用的合约方法名称
    --runtime-type：指定合约执行虚拟机环境，包含：EVM
  ...
  - 创建wasm合约
    ...
    ```
      ...
      > 如下返回表示成功：
      >
      > response: message:"OK" contract_result:<result:"\n\004fact\022\0031.0\030\002*<\n\026yz-org1.yzchain.org\020\001\032 F]\334,\005O\200\272\353\213\274\375nT\026%K\r\314\362\361\253X\356*2\377\216\250kh\031" message:"OK" > tx_id:"991a1c00369e4b76853dadf410182bcdfc86062f8cf1478f93482ba9000191d7"
  ...
  - 创建evm合约
    ...
    ```
      ...
      > response: message:"OK" contract_result:<result:"\n(532c238cec7071ce8655aba07e50f9fb16f72ca1\022\0031.0\030\005*<\n\026yz-org1.yzchain.org\020\001\032 F]\334,\005O\200\272\353\213\274\375nT\026%K\r\314\362\361\253X\356*2\377\216\250kh\031" message:"OK" > tx_id:"e2af1241ff464d47b869a69ce8a615df50da57d3faff4754ad6e45b9f914b938"
  ...
  - 升级合约
    ...
    ```sh
    $ ./yzc client contract user upgrade \
    --contract-name=fact \
    --runtime-type=WASMER \
    --byte-code-path=./testdata/claim-wasm-demo/rust-fact-2.0.0.wasm \
    --version=2.0 \
    ...
    ```
      ...
      > upgrade contract resp: message:"OK" contract_result:<result:"\n\004fact\022\0032.0\030\002*<\n\026yz-org1.yzchain.org\020\001\032 F]\334,\005O\200\272\353\213\274\375nT\026%K\r\314\362\361\253X\356*2\377\216\250kh\031" message:"OK" > tx_id:"d89df9fcd87f4071972fdabdf3003a349250a94893fb43899eac4d68e7855d52" 

  - 冻结合约
  
    ```sh
    $ ./yzc client contract user freeze \
    --contract-name=fact \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --admin-key-file-paths=./testdata/crypto-config/yz-org1.yzchain.org/user/admin1/admin1.tls.key,./testdata/crypto-config/yz-org2.yzchain.org/user/admin1/admin1.tls.key,./testdata/crypto-config/yz-org3.yzchain.org/user/admin1/admin1.tls.key \
    --admin-crt-file-paths=./testdata/crypto-config/yz-org1.yzchain.org/user/admin1/admin1.tls.crt,./testdata/crypto-config/yz-org2.yzchain.org/user/admin1/admin1.tls.crt,./testdata/crypto-config/yz-org3.yzchain.org/user/admin1/admin1.tls.crt \
    ...
    >
    > freeze contract resp: message:"OK" contract_result:<result:"{\"name\":\"fact\",\"version\":\"3.0\",\"runtime_type\":2,\"status\":1,\"creator\":{\"org_id\":\"yz-org1.yzchain.org\",\"member_type\":1,\"member_info\":\"Rl3cLAVPgLrri7z9blQWJUsNzPLxq1juKjL/jqhraBk=\"}}" message:"OK" > tx_id:"09841775173548ad9a8a39e2987a4f5115d59d50dd3448e8b09a83624dee5367"

  - 解冻合约
  
    ```sh
    $ ./yzc client contract user unfreeze \
    --contract-name=fact \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --admin-key-file-paths=./testdata/crypto-config/yz-org1.yzchain.org/user/admin1/admin1.tls.key,./testdata/crypto-config/yz-org2.yzchain.org/user/admin1/admin1.tls.key,./testdata/crypto-config/yz-org3.yzchain.org/user/admin1/admin1.tls.key \
    --admin-crt-file-paths=./testdata/crypto-config/yz-org1.yzchain.org/user/admin1/admin1.tls.crt,./testdata/crypto-config/yz-org2.yzchain.org/user/admin1/admin1.tls.crt,./testdata/crypto-config/yz-org3.yzchain.org/user/admin1/admin1.tls.crt \
    ...
    >
    > unfreeze contract resp: message:"OK" contract_result:<result:"{\"name\":\"fact\",\"version\":\"3.0\",\"runtime_type\":2,\"creator\":{\"org_id\":\"yz-org1.yzchain.org\",\"member_type\":1,\"member_info\":\"Rl3cLAVPgLrri7z9blQWJUsNzPLxq1juKjL/jqhraBk=\"}}" message:"OK" > tx_id:"fccf024450c140dea999cc46ad24d381a679ce2142bd48b2a829abcd4f099866"

  - 吊销合约
  
    ```sh
    $ ./yzc client contract user revoke \
    --contract-name=fact \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --admin-key-file-paths=./testdata/crypto-config/yz-org1.yzchain.org/user/admin1/admin1.tls.key,./testdata/crypto-config/yz-org2.yzchain.org/user/admin1/admin1.tls.key,./testdata/crypto-config/yz-org3.yzchain.org/user/admin1/admin1.tls.key \
    --admin-crt-file-paths=./testdata/crypto-config/yz-org1.yzchain.org/user/admin1/admin1.tls.crt,./testdata/crypto-config/yz-org2.yzchain.org/user/admin1/admin1.tls.crt,./testdata/crypto-config/yz-org3.yzchain.org/user/admin1/admin1.tls.crt \
    ...
    > 
    > revoke contract resp: message:"OK" contract_result:<result:"{\"name\":\"fact\",\"version\":\"3.0\",\"runtime_type\":2,\"status\":2,\"creator\":{\"org_id\":\"yz-org1.yzchain.org\",\"member_type\":1,\"member_info\":\"Rl3cLAVPgLrri7z9blQWJUsNzPLxq1juKjL/jqhraBk=\"}}" message:"OK" > tx_id:"d971b57cf12c46ff8fe0d4f5897634c644fb802998f44360bb130f27ff54a10a"
...
<span id="chainConfig.addrFromCert"></span>
  - 用户证书计算出用户的地址
    ...
    ```sh
    $ ./yzc cert userAddr --cert-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.sign.crt
    ```
...
<span id="chainConfig.dposMint"></span>
  - 增发Token
    ...
    ```sh
    $ ./yzc client contract system mint \
    --amount=100000000 \
    --address=ADZTrzVF9SuvQqmn9YTAJiwnLCnXonMTj6Bq1HRiwVnR \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --chain-id=chain1 \
    --user-tlscrt-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.crt \
    --user-tlskey-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.key \
    --user-signcrt-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.sign.crt \
    --user-signkey-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.sign.key \
    --sync-result=true
    ```
...
<span id="chainConfig.dposTransfer"></span>
  ...
  - 转账
    ...
    ```sh
    $ ./yzc client contract system transfer \
    --amount=100000000 \
    --address=ADZTrzVF9SuvQqmn9YTAJiwnLCnXonMTj6Bq1HRiwVnR \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
...
<span id="chainConfig.dposBalanceOf"></span>
  - 查询余额
    ...
    ```sh
    $ ./yzc client contract system balance-of \
    --address=ADZTrzVF9SuvQqmn9YTAJiwnLCnXonMTj6Bq1HRiwVnR \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --chain-id=chain1 \
...
<span id="chainConfig.dposOwner"></span>
  - 查询合约管理地址
    ...
    ```sh
    $ ./yzc client contract system owner \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --chain-id=chain1 \
    --user-tlscrt-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.crt \
...
<span id="chainConfig.dposDecimals"></span>
  - 查询ERC20合约的精度
    ...
    ```sh
    $ ./yzc client contract system decimals \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --chain-id=chain1 \
    --user-tlscrt-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.crt \
...
<span id="chainConfig.dposTotalAmount"></span>
  - 查询 Token 总供应量
    ...
    ```sh
    $ ./yzc client contract system total \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --chain-id=chain1 \
    --user-tlscrt-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.crt \
...
<span id="chainConfig.dposCandidates"></span>
  - 查询所有的候选人
    ...
    ```sh
    $ ./yzc client contract system all-candidates \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --chain-id=chain1 \
    --user-tlscrt-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.crt \
...
<span id="chainConfig.dposValidatorInfo"></span>
  ...
  - 查询指定验证人的信息
    ...
    ```sh
    $ ./yzc client contract system get-validator \
    --address=ADZTrzVF9SuvQqmn9YTAJiwnLCnXonMTj6Bq1HRiwVnR \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --chain-id=chain1 \
...
<span id="chainConfig.dposDelegate"></span>
  - 抵押权益到验证人
    ...
    ```sh
    $ ./yzc client contract system delegate \
    --address=ADZTrzVF9SuvQqmn9YTAJiwnLCnXonMTj6Bq1HRiwVnR \
    --amount=100000000 \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org5.yzchain.org \
    --chain-id=chain1 \
    --user-tlscrt-file-path=./testdata/crypto-config/yz-org5.yzchain.org/user/client1/client1.tls.crt \
    --user-tlskey-file-path=./testdata/crypto-config/yz-org5.yzchain.org/user/client1/client1.tls.key \
    --user-signcrt-file-path=./testdata/crypto-config/yz-org5.yzchain.org/user/client1/client1.sign.crt \
    --user-signkey-file-path=./testdata/crypto-config/yz-org5.yzchain.org/user/client1/client1.sign.key \
    --sync-result=true
    ```
...
<span id="chainConfig.dposUserDelegations"></span>
  - 查询指定地址的抵押信息
    ...
    ```sh
    $ ./yzc client contract system get-delegations-by-address \
    --address=ADZTrzVF9SuvQqmn9YTAJiwnLCnXonMTj6Bq1HRiwVnR \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --chain-id=chain1 \
...
<span id="chainConfig.dposUserDelagationWithValidator"></span>
  - 查询指定地址的抵押信息
    ...
    ```sh
    $ ./yzc client contract system get-user-delegation-by-validator \
...
<span id="chainConfig.dposUndelegate"></span>
  - 从验证人解除抵押的权益
    ...
    ```sh
    $ ./yzc client contract system undelegate \
    --address=ADZTrzVF9SuvQqmn9YTAJiwnLCnXonMTj6Bq1HRiwVnR \
    --amount=100000000 \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org5.yzchain.org \
...
<span id="chainConfig.dposEpochInfoByID"></span>
  ...
  - 查询指定世代信息
    ...
    ```sh
    $ ./yzc client contract system read-epoch-by-id \
    --epoch-id=1 \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --chain-id=chain1 \
...
<span id="chainConfig.dposCurrEpochInfo"></span>
  - 查询当前世代信息
    ...
    ```sh
    $ ./yzc client contract system read-latest-epoch \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --chain-id=chain1 \
    --user-tlscrt-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.crt \
...
<span id="chainConfig.dposSetNodeID"></span>
  - Stake合约中设置验证人的NodeID
    ...
    ```sh
    $ ./yzc client contract system set-node-id \
...
<span id="chainConfig.dposGetNodeID"></span>
  ...
  - Stake合约中查询验证人的NodeID
    ...
    ```sh
    $ ./yzc client contract system get-node-id \
...
<span id="chainConfig.dposMinSelfDelegation"></span>
  - 查询验证人节点的最少自我抵押数量
    ...
    ```sh
    $ ./yzc client contract system min-self-delegation \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --chain-id=chain1 \
    --user-tlscrt-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.crt \
...
<span id="chainConfig.dposValidatorNumInEpoch"></span>
  - 查询世代中的验证人数
    ...
    ```sh
    $ ./yzc client contract system validator-number \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --chain-id=chain1 \
    --user-tlscrt-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.crt \
...
<span id="chainConfig.dposBlockNumEachEpoch"></span>
  - 查询世代中的区块数量
    ...
    ```sh
    $ ./yzc client contract system epoch-block-number \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --chain-id=chain1 \
    --user-tlscrt-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.crt \
...
<span id="chainConfig.dposSystemStakeAddr"></span>
  - 查询Stake合约的系统地址
    ...
    ```sh
    $ ./yzc client contract system system-address \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --chain-id=chain1 \
    --user-tlscrt-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.crt \
...
<span id="chainConfig.dposCompeleteUndeleteBlockNum"></span>
  - 查询收到解质押退款间隔的世代数
    ...
    ```sh
    $ ./yzc client contract system unbonding-epoch-number \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --chain-id=chain1 \
    --user-tlscrt-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.crt \
...
#### 查询链上数据
  ...
  - 根据区块高度查询链上未归档区块
    ...
    ```sh
    ./yzc query block-by-height [blockheight] \
    --chain-id=chain1 \
    --sdk-conf-path=./testdata/sdk_config.yml
    ```
  ...
  - 根据区块hash查询链上未归档区块
    ...
    ```sh
    ./yzc query block-by-hash [blockhash] \
    --chain-id=chain1 \
    --sdk-conf-path=./testdata/sdk_config.yml
    ```
...
<span id="chainConfig.updateBlockInterval"></span>
  - 更新出块时间
    ...
    ```sh
    ./yzc client chainconfig block updateblockinterval \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --admin-key-file-paths=./testdata/crypto-config/yz-org1.yzchain.org/user/admin1/admin1.tls.key,./testdata/crypto-config/yz-org2.yzchain.org/user/admin1/admin1.tls.key,./testdata/crypto-config/yz-org3.yzchain.org/user/admin1/admin1.tls.key \
    --admin-crt-file-paths=./testdata/crypto-config/yz-org1.yzchain.org/user/admin1/admin1.tls.crt,./testdata/crypto-config/yz-org2.yzchain.org/user/admin1/admin1.tls.crt,./testdata/crypto-config/yz-org3.yzchain.org/user/admin1/admin1.tls.crt \
    --block-interval 1000
...
<span id="chainConfig.addOrgRootCA"></span>
  - 增加组织根证书
    ...
    ```sh
    ./yzc client chainconfig trustroot add \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --user-tlscrt-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.crt \
    --user-tlskey-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.key \
    ...
    --admin-crt-file-paths=./testdata/crypto-config/yz-org1.yzchain.org/user/admin1/admin1.tls.crt,./testdata/crypto-config/yz-org2.yzchain.org/user/admin1/admin1.tls.crt,./testdata/crypto-config/yz-org3.yzchain.org/user/admin1/admin1.tls.crt \
    --admin-key-file-paths=./testdata/crypto-config/yz-org1.yzchain.org/user/admin1/admin1.tls.key,./testdata/crypto-config/yz-org2.yzchain.org/user/admin1/admin1.tls.key,./testdata/crypto-config/yz-org3.yzchain.org/user/admin1/admin1.tls.key \
    --trust-root-org-id=yz-org5.yzchain.org \
    --trust-root-path=./testdata/crypto-config/yz-org5.yzchain.org/ca/ca.crt
    ```
...
<span id="chainConfig.delOrgRootCA"></span>
  - 删除组织根证书
    ...
    ```sh
    ./yzc client chainconfig trustroot remove \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --user-tlscrt-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.crt \
    --user-tlskey-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.key \
...
<span id="chainConfig.updateOrgRootCA"></span>
  - 更新组织根证书
    ...
    ```sh
    ./yzc client chainconfig trustroot update \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --user-tlscrt-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.crt \
    --user-tlskey-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.key \
...
<span id="chainConfig.addConsensusNodeOrg"></span>
  - 添加共识节点Org
    ...
    ```sh
    ./yzc client chainconfig consensusnodeorg add \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --user-tlscrt-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.crt \
    --user-tlskey-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.key \
    ...
    --node-org-id=yz-org5.yzchain.org
    ```
...
<span id="chainConfig.delConsensusNodeOrg"></span>
  - 删除共识节点Org
    ...
    ```sh
    ./yzc client chainconfig consensusnodeorg remove \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --user-tlscrt-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.crt \
    --user-tlskey-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.key \
...
<span id="chainConfig.updateConsensusNodeOrg"></span>
  - 更新共识节点Org
    ...
    ```sh
    ./yzc client chainconfig consensusnodeorg update \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --user-tlscrt-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.crt \
    --user-tlskey-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.key \
...
<span id="chainConfig.addConsensusNodeId"></span>
  - 添加共识节点Id
    ...
    ```sh
    ./yzc client chainconfig consensusnodeid add \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --user-tlscrt-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.crt \
    --user-tlskey-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.key \
    ...
    --admin-key-file-paths=./testdata/crypto-config/yz-org1.yzchain.org/user/admin1/admin1.tls.key,./testdata/crypto-config/yz-org2.yzchain.org/user/admin1/admin1.tls.key,./testdata/crypto-config/yz-org3.yzchain.org/user/admin1/admin1.tls.key \
    --node-id=QmcQHCuAXaFkbcsPUj7e37hXXfZ9DdN7bozseo5oX4qiC4 \
    --node-org-id=yz-org1.yzchain.org
    ```
...
<span id="chainConfig.delConsensusNodeId"></span>
  - 删除共识节点Id
    ...
    ```sh
    ./yzc client chainconfig consensusnodeid remove \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --user-tlscrt-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.crt \
    --user-tlskey-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.key \
...
<span id="chainConfig.updateConsensusNodeId"></span>
  - 更新共识节点Id
    ...
    ```sh
    ./yzc client chainconfig consensusnodeid update \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --org-id=yz-org1.yzchain.org \
    --user-tlscrt-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.crt \
    --user-tlskey-file-path=./testdata/crypto-config/yz-org1.yzchain.org/user/client1/client1.tls.key \
...
#### 归档&恢复功能
  ...
  yzc的归档功能是指将链上数据转移到独立存储上，归档后的数据具备可查询、可恢复到链上的特性。<br>
  为了保持数据一致性和防止误操作，yzc实现了分布式锁，同一时刻只允许一个yzc进程进行转储。<br>
  yzc支持增量转储和恢复、断点中继转储和恢复，中途退出不影响数据一致性。<br><br>
  主要参数说明如下：
  ...
  ```sh
    ...
    --blocks：指定本次要转储的块数量，注意：对于target和blocks这两个参数，yzc会就近原则采用先符合条件的参数
  ...
  - 恢复，将链下的链数据恢复到链上，需要权限：sdk配置文件中设置与归档节点同组织的[admin用户](#sdkConfig)
    ...
    ```sh
    ./yzc archive restore --type=mysql \
    --dest=root:password:localhost:3306 \
  ...
  - 根据区块高度查询链下已归档区块
    ...
    ```sh
    ./yzc archive query block-by-height [blockheight] \
    --chain-id=chain1 \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --type=mysql \
    --dest=root:password:localhost:3306
  ...
  - 根据区块hash查询链下已归档区块
    ...
    ```sh
    ./yzc archive query block-by-hash [blockhash] \
    --chain-id=chain1 \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --type=mysql \
    --dest=root:password:localhost:3306
  ...
  - 根据txid查询链下已归档区块
    ...
    ```sh
    ./yzc archive query block-by-txid [txid] \
    --chain-id=chain1 \
    --sdk-conf-path=./testdata/sdk_config.yml \
    --type=mysql \
    --dest=root:password:localhost:3306
