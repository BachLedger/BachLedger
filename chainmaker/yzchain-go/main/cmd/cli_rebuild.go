...
func RebuildDbsCMD() *cobra.Command {
	rebuildDbsCmd := &cobra.Command{
		Use:   "rebuild-dbs",
		Short: "RebuildDbs yzchain",
		Long:  "RebuildDbs yzchain",
		RunE: func(cmd *cobra.Command, _ []string) error {
			initLocalConfig(cmd)
			backupDbs(rebuildChainId, needVerify)
			rebuildDbsStart()
			fmt.Println("yzchain exit")
			return nil
		},
	}
...
func rebuildDbsStart() {
	...
	if err := chainMakerServer.InitForRebuildDbs(chainId); err != nil {
		log.Errorf("yzchain server init failed, %s", err.Error())
		return
	}

	// init rpc server
	//rpcServer, err := rpcserver.NewRPCServer(yzchainServer)
	//if err != nil {
	...
	if err := chainMakerServer.StartForRebuildDbs(needVeri); err != nil {
		log.Errorf("yzchain server startup failed, %s", err.Error())
		return
	}
	...
	errC := <-errorC
	if errC != nil {
		log.Error("yzchain encounters error ", errC)
	}
