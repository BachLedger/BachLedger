...
import (
	...
	coreCommon "chainmaker.org/chainmaker-go/module/core/common"
...
func (bc *Blockchain) Init() (err error) {
	baseModules := []map[string]func() error{
		...
		// init chain config , must later than store module
		{moduleNameChainConf: bc.initChainConf},
		// init tx filter , must later than store module
...
func (bc *Blockchain) initCache() (err error) {
	...
	if bc.lastBlock != nil {
		...
		genesisBlock.AdditionalData = &common.AdditionalData{
			ExtraData: make(map[string][]byte),
		}

		genesisBlock.AdditionalData.ExtraData[coreCommon.PreTempLinkingFieldKey] = nil
		genesisBlock.AdditionalData.ExtraData[coreCommon.TempLinkingFieldKey], err =
			coreCommon.GenerateTxBatchHashForTempLinking(
				genesisBlock.Txs, "SHA256")
		if err != nil {
			bc.log.Errorf("Encountering err when generate temp link for genesis block. error: %s", err)
		}
	...
	bc.commitedLedgerCache = cache.NewLedgerCache(bc.chainId)
	bc.ledgerCache.SetLastBlock(bc.lastBlock)
	bc.commitedLedgerCache.SetLastBlock(bc.lastBlock)
	bc.proposalCache = cache.NewProposalCache(bc.chainConf, bc.ledgerCache, bc.commitedLedgerCache, bc.log)
...
func (bc *Blockchain) initCore() (err error) {
	...
	coreEngineConfig := &providerConf.CoreEngineConfig{
		...
		CommitedLedgerCache: bc.commitedLedgerCache,
