...
func TestProcessorProcessBlockMsg(t *testing.T) {
	...
	require.EqualValues(t, 101, ledger.GetLastBlock().Header.BlockHeight)
	...
	for i := 1; i <= 3; i++ {
		...
		require.EqualValues(t, 101+i, ledger.GetLastBlock().Header.BlockHeight)
	}
...
func TestDataDetection(t *testing.T) {
	...
	ledger.SetLastBlock(&commonPb.Block{Header: &commonPb.BlockHeader{BlockHeight: 102}})
	ret, err = processor.handler(&DataDetection{})
	require.Nil(t, ret)
	require.NoError(t, err)
	require.EqualValues(t, 2, len(processor.queue))
	...
	ledger.SetLastBlock(&commonPb.Block{Header: &commonPb.BlockHeader{BlockHeight: 120}})
	ret, err = processor.handler(&DataDetection{})
	require.Nil(t, ret)
	require.NoError(t, err)
	require.EqualValues(t, 0, len(processor.queue))
