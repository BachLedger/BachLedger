...
func newMockLedgerCache(ctrl *gomock.Controller, blk *commonPb.Block) protocol.LedgerCache {
	...
	lastCommitBlk := blk
	mockLedger.EXPECT().GetLastBlock().DoAndReturn(func() *commonPb.Block {
	...
	mockLedger.EXPECT().SetLastBlock(gomock.Any()).DoAndReturn(func(blk *commonPb.Block) {
		lastCommitBlk = blk
	}).AnyTimes()
	return mockLedger
}
