...
func (sf TxSchedulerFactory) NewTxScheduler(vmMgr protocol.VmManager, chainConf protocol.ChainConf,
	storeHelper conf.StoreHelper, commitedLedgerCache protocol.LedgerCache) protocol.TxScheduler {
	if chainConf.ChainConfig().Scheduler != nil && chainConf.ChainConfig().Scheduler.EnableEvidence {
		return newTxSchedulerEvidence(vmMgr, chainConf, storeHelper, commitedLedgerCache)
	}
	return newTxScheduler(vmMgr, chainConf, storeHelper, commitedLedgerCache)
}
