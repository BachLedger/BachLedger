...
type BlockVerifierImpl struct {
	...
	commitedLedgerCache protocol.LedgerCache     // ledger cache
...
func (v *BlockVerifierImpl) validateBlock(block, lastBlock *commonpb.Block, mode protocol.VerifyMode) (
	...
	*common.RwSetVerifyFailTx, error) {
	...
	lastBlockHash := lastBlock.AdditionalData.ExtraData[common.TempLinkingFieldKey]
	err = common.CheckPreBlockWithTempHash(block, lastBlockHash, proposedHeight)
	if err != nil {
		return nil, nil, timeLasts, nil, err
	}
...
func (v *BlockVerifierImpl) verifyVoteSig(block *commonpb.Block) error {
	return consensus.VerifyBlockSignatures(v.chainConf, v.ac, v.blockchainStore, block, v.commitedLedgerCache)
}
...
func (v *BlockVerifierImpl) verifyRepeat(block *commonpb.Block, startTick int64,
	...
	if consensuspb.ConsensusType_SOLO != v.chainConf.ChainConfig().Consensus.Type || isSqlDb {
		...
		lastBlock, _ := v.proposalCache.GetProposedBlockByHashAndHeight(
			block.AdditionalData.ExtraData[common.PreTempLinkingFieldKey],
		...
		cutBlocks := v.proposalCache.KeepProposedBlock(
			lastBlock.AdditionalData.ExtraData[common.TempLinkingFieldKey],
