...
func (bp *BlockProposerImpl) shouldProposeByBFT(height uint64) bool {
	...
	committedBlock := bp.ledgerCache.GetLastBlock()
...
func (bp *BlockProposerImpl) proposeBlock() {
	...
	lastBlock := bp.ledgerCache.GetLastBlock()
	if lastBlock == nil {
		bp.log.Errorf("no committed block found")
		return
	}
	...
	go bp.proposing(proposingHeight, lastBlock.AdditionalData.ExtraData[common.TempLinkingFieldKey])
	// #DEBUG MODE#
...
func (bp *BlockProposerImpl) proposing(height uint64, preHash []byte) *commonpb.Block {
	...
	var (
		...
		fetchTotalLasts     int64 // The total consumed time
	...
	bp.log.Errorf("perf: propose blk[%d], now[%d]", height, time.Now().UnixMicro())
...
func (bp *BlockProposerImpl) dealProposalRequestWithProposalCache(
	...
	if bytes.Equal(selfProposedBlock.AdditionalData.ExtraData[common.PreTempLinkingFieldKey], preHash) {
