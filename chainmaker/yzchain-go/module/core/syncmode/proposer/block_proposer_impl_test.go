...
func TestProposeStatusChange(t *testing.T) {
	...
	commitedLedgerCache := cache.NewLedgerCache(chainId)
	proposedCache := cache.NewProposalCache(nil, ledgerCache, commitedLedgerCache, log)
	...
	ledgerCache.SetLastBlock(createNewTestBlock(0))
	...
	txScheduler.EXPECT().GetResultMaps(gomock.Any(), gomock.Any(), gomock.Any()).AnyTimes()

	cmLogger := logger.GetLoggerByChain(logger.MODULE_CORE, chainId)
...
func TestProposeBlock(t *testing.T) {
	...
	ledgerCache.EXPECT().GetLastBlock().Return(b1).AnyTimes()
