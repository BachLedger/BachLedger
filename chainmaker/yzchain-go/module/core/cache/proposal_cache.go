...
func NewProposalCache(
	chainConf protocol.ChainConf,
	ledgerCache protocol.LedgerCache,
	committedLedgerCache protocol.LedgerCache,
	...
	pc := &ProposalCache{
		...
		commitedLedgerCache: committedLedgerCache,
		logger:              logger,
	}
	return pc
}
...
func (pc *ProposalCache) GetProposedBlockByHashAndHeight(tempHash []byte, height uint64) (
	*commonpb.Block, map[string]*commonpb.TxRWSet) {
	...
	if proposedBlocks, ok := pc.lastProposedBlock[height]; ok {
		for _, proposedBlock := range proposedBlocks {
			if bytes.Equal(
				proposedBlock.block.AdditionalData.ExtraData[common.TempLinkingFieldKey], tempHash) {
...
func (pc *ProposalCache) GetProposedBlockByTempHashAndHeight(tempHash []byte, height uint64) (*commonpb.Block, map[string]*commonpb.TxRWSet) {
	if tempHash == nil {
		return nil, nil
	}
	// starting lock when we read the map
...
func (pc *ProposalCache) SetProposedBlock(b *commonpb.Block, rwSetMap map[string]*commonpb.TxRWSet,
	...
	height := b.Header.BlockHeight
	currentHeight, err := pc.commitedLedgerCache.CurrentHeight()
	if err != nil {
		return err
	}
	...
	fingerPrint := utils.CalcBlockFingerPrint(b)
	pc.logger.Infof("CalcBlockFingerPrint in SetProposedBlock: "+
		"height: %d, blockHeight: %d, fingerPrint: %s", height, currentHeight, fingerPrint)
	bs := &blockProposal{
		block:                b,
		rwSetMap:             rwSetMap,
	...
	pc.logger.DebugDynamic(func() string {
		return fmt.Sprintf(
			"set proposed block, height: %d, fingerPrint:%s, hash: %x, txHash: %x",
			b.Header.BlockHeight, string(fingerPrint), b.Header.BlockHash,
			b.AdditionalData.ExtraData[common.TempLinkingFieldKey])
	})
...
func (pc *ProposalCache) ClearTheBlock(block *commonpb.Block) {
	...
	if proposedBlocks, ok := pc.lastProposedBlock[block.Header.BlockHeight]; ok {
		...
		pc.logger.DebugDynamic(func() string {
			return fmt.Sprintf(
				"clear the block from proposal cache, height: %d, fingerPrint:%s, hash: %x, tempHash: %x",
				block.Header.BlockHeight, string(fingerPrint), block.Header.BlockHash,
				block.AdditionalData.ExtraData[common.TempLinkingFieldKey])
		})
	}
}
...
func (pc *ProposalCache) KeepProposedBlock(hash []byte, height uint64) []*commonpb.Block {
	...
	if proposedBlocks, ok := pc.lastProposedBlock[height]; ok {
		for _, proposedBlock := range proposedBlocks {
			if !bytes.Equal(hash,
				proposedBlock.block.AdditionalData.ExtraData[common.TempLinkingFieldKey]) {
				...
				pc.logger.DebugDynamic(func() string {
					return fmt.Sprintf(
						"remove proposed block from proposal cache, "+
							"height: %d, fingerPrint:%s, hash: %x, txhash: %x",
						proposedBlock.block.Header.BlockHeight, fingerPrint,
						proposedBlock.block.Header.BlockHash,
						proposedBlock.block.AdditionalData.ExtraData[common.TempLinkingFieldKey])
				})
			}
		}
