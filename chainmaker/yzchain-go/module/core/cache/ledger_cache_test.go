...
func TestLedger(t *testing.T) {
	...
	b := ledgerCache.GetLastBlock()
	b.Header.BlockHeight = 100
	ledgerCache.SetLastBlock(b)
...
func (m *moduleA) updateBlock(block *commonpb.Block) {
	m.ledgerCache.SetLastBlock(block)
}

func (m *moduleA) getBlock() *commonpb.Block {
	return m.ledgerCache.GetLastBlock()
}
...
/*
 * test unit ledger cache GetLastBlock func
 */
func TestLedgerCache_GetLastCommittedBlock(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := lc.GetLastBlock(); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("GetLastBlock() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
/*
 * test unit ledger cache SetLastBlock func
 */
