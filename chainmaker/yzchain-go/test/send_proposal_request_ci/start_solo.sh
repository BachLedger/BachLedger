...
if [ -z ${pid} ]; then
  nohup ./yzchain start -c ../config/yz-org1-solo/yzchain.yml ci-solo >./panic.log &
  echo "wx-solo yzchain is starting, pls check log..."
else
  echo "wx-solo yzchain is already started"
fi
