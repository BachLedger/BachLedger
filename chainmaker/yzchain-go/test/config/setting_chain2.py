...
def UpdateSetting():
    ...
    gl.ADMIN_KEY_FILE_PATHS = ','.join([f'{gl.CRYPTO_CONFIG_PATH}/yz-org{i}.chainmaker.org/keys/user/admin/admin.key'
                                        for i in range(1, 4)])
    gl.ADMIN_ORG_IDS = ','.join([f'yz-org{i}.chainmaker.org' for i in range(1, 4)])

    gl.WASM_APTH = r'../../testdata/'
    gl.SDK_PATH = r'../config/'
    gl.ACCOUNT_TYPE = "pwk"
    gl.USER_B_KEY = r"yz-org2.yzchain.org/keys/user/admin/admin.key"
