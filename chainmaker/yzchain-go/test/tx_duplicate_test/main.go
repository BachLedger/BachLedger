...
var (
	// Solo
	caPaths = [][]string{
		{"./config/crypto-config/yz-org1.yzchain.org/ca"},
	}
	userKeyPaths = []string{
		"./config/crypto-config/yz-org1.yzchain.org/user/client1/client1.sign.key",
	}
	userCrtPaths = []string{
		"./config/crypto-config/yz-org1.yzchain.org/user/client1/client1.sign.crt",
	}
	IPs = []string{
		"127.0.0.1",
	}
...
func getUserSK(orgIDNum int, keyPath, certPath string) (crypto.PrivateKey, *acPb.Member) {
	...
	sender := &acPb.Member{
		OrgId:      fmt.Sprintf("yz-org%d.chainmaker.org", orgIDNum),
		MemberInfo: file2,
		//IsFullCert: true,
	}
	return sk3, sender
