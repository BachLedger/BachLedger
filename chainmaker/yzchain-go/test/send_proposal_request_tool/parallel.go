...
func createContractCMD() *cobra.Command {
	...
	flags.Int32VarP(&runTime, "run-time", "m", int32(commonPb.RuntimeType_EVM), "specify run time")
...
func upgradeContractCMD() *cobra.Command {
	...
	flags := cmd.Flags()
	flags.StringVarP(&wasmPath, "wasm-path", "w", "../wasm/counter-go.wasm", "specify wasm path")
	flags.Int32VarP(&runTime, "run-time", "R", int32(commonPb.RuntimeType_EVM), "specify run time")
