...
vm:
  go:
    ...
    data_mount_path: ../data/yz-org1.yzchain.org/go
    # 日志挂载路径
    log_mount_path: ../log/yz-org1.yzchain.org/go
    # yzchain和合约引擎之间的通信协议（可选tcp/uds）
    protocol: tcp
    # 如果需要自定义高级配置，请将vm.yml文件放入dockervm_config_path中，优先级：yzchain.yml > vm.yml > 默认配置
    ...
    # 合约引擎服务器配置 (与yzchain交互，进行交易请求、合约请求等交互)
    contract_engine:
...
--net=host \
--privileged \
-v /data/yzchain/node1/go:/mount \
-v /data/yzchain/node1/log:/log \
...
正常启动yzchain: ./yzchain start -c config 