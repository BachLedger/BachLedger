...
func byteAction(ctx *instructionsContext) ([]byte, error) {
	...
	var b byte
	if ctx.storage.ExternalStorage.GetCurrentBlockVersion() <= 2210 {
		//<= v2.2.1 use old code, though it's fault
		b = x.ByteAt(int(i.Uint64()))
	} else {
		//new logic read the opsite index
		bnBytes := x.Int.Bytes()
		if len(bnBytes) == 0 && ctx.storage.ExternalStorage.GetCurrentBlockVersion() > 2212 {
			bnBytes = []byte{0}
		}
