...
func addressAction(ctx *instructionsContext) ([]byte, error) {
	ctx.stack.Push(ctx.environment.Contract.Address.Clone())
	return nil, nil
}
...
func returnDataCopyAction(ctx *instructionsContext) ([]byte, error) {
	...
	if !end.IsUint64() || end.Uint64() > uint64(len(ctx.lastReturn)) {
		return nil, utils.ErrReturnDataCopyOutOfBounds
	}
...
func chainIdAction(ctx *instructionsContext) ([]byte, error) {
	...
	if version < params.V2218 || version == params.V2300 || version == params.V2030100 {
		...
		return nil, utils.InvalidOpCode(byte(opcodes.CHAINID))
	}

	chainId := evmutils.New(0)
	chainId.SetBytes([]byte(ctx.environment.Cfg.ChainId))
	//ctx.stack.Push(chainId.Clone())
...
func selfBalanceAction(ctx *instructionsContext) ([]byte, error) {
	...
	if version < params.V2218 || version == params.V2300 || version == params.V2030100 {
		//2300 and 2310 have been released, but 2217 has found bugs, so versions before 2218, as well as 2300 and 2310
		//that have been released, use the old logic, and other versions use the new logic
		return nil, utils.InvalidOpCode(byte(opcodes.SELFBALANCE))
	}

	//gasUsed := ctx.environment.Block.GasLimit.Uint64() - ctx.GetGasLeft()
	//ctx.storage.ExternalStorage.GetBalance(ctx.environment.Contract.Address, gasUsed)

	//b, err := ctx.storage.ExternalStorage.GetBalance(ctx.environment.Contract.Address)
	//if err != nil {
	//	return nil, err
	//}
	b, _ := ctx.storage.ExternalStorage.GetBalance(ctx.environment.Contract.Address)
...
func baseFeeAction(ctx *instructionsContext) ([]byte, error) {
	...
	if version < params.V2218 || version == params.V2300 || version == params.V2030100 {
		//2300 and 2310 have been released, but 2217 has found bugs, so versions before 2218, as well as 2300 and 2310
		//that have been released, use the old logic, and other versions use the new logic
		return nil, utils.InvalidOpCode(byte(opcodes.BASEFEE))
	}

	ctx.stack.Push(ctx.environment.Transaction.BaseFee.Clone())
	return nil, nil
}
