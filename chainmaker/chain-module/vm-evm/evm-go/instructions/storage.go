...
func sLoadAction(ctx *instructionsContext) ([]byte, error) {
	...
	if version < params.V2218 || version == params.V2300 || version == params.V2030100 {
		...
		v, err = ctx.storage.SLoad2218(ctx.environment.Contract.Address, k)
		if err != nil {
			return nil, err
		}

		addrStr := hex.EncodeToString(ctx.environment.Contract.Address.Bytes())
		keyStr := hex.EncodeToString(k.Bytes())
		ctx.storage.ResultCache.CachedData.Set(addrStr, keyStr, v)
	}
