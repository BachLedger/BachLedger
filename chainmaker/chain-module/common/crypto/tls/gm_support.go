...
const (
	ModeGMSSLOnly  = "GMSSLOnly"  // only support GMSSL
	ModeAutoSwitch = "AutoSwitch" // support GMSSL/TLS auto switch
)

type GMSupport struct {
	WorkMode      string // working mode
	EncCertEnable bool   // enable encryption certificate
}

func NewGMSupport() *GMSupport {
	return &GMSupport{WorkMode: ModeGMSSLOnly, EncCertEnable: true}
}
...
func matchKeyCert(keyDERBlock *pem.Block, certDERBlock []byte) (crypto.PrivateKey, error) {
	...
	pubKey := x509Cert.PublicKey.ToStandardKey()
