...
func parseSANExtension(value []byte) (dnsNames, emailAddresses []string,
	ipAddresses []net.IP, uris []*url.URL, err error) {
...
func parsePublicKey(algo PublicKeyAlgorithm, keyData *publicKeyInfo) (bccrypto.PublicKey, error) {
	...
	case ECDSA:
		...
		if namedCurveOID.Equal(oidNamedCurveSm2) {
			...
			return &bcsm2.PublicKey{K: pk}, nil
		}
	...
	case SM2:
		...
		pk, err := tjx509.ParseSm2PublicKey(asn1Data)
		if err != nil {
			pk, err = asym.ParseSM2PublicKey(asn1Data)
			if err != nil {
...
func parseNameConstraintsExtension(out *Certificate, e pkix.Extension) (unhandled bool, err error) {
	...
	getValues := func(subtrees cryptobyte.String) (
		dnsNames []string, ips []*net.IPNet, emails, uriDomains []string, err error) {
		for !subtrees.Empty() {
	...
	if out.PermittedDNSDomains,
		out.PermittedIPRanges,
		out.PermittedEmailAddresses,
		out.PermittedURIDomains,
		err = getValues(permitted); err != nil {
		return false, err
	}
	if out.ExcludedDNSDomains,
		out.ExcludedIPRanges,
		out.ExcludedEmailAddresses,
		out.ExcludedURIDomains,
		err = getValues(excluded); err != nil {
		return false, err
	}
...
// signature algorithm.
func signingParamsForPublicKey(pub interface{},
...
func CreateCertificate(rand io.Reader, template,
	...
	hashFunc, signatureAlgorithm, err := signingParamsForPublicKey(
		key.Public(),
