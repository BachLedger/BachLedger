...
func (pool *batchTxPool) AddTx(tx *commonPb.Transaction, source protocol.TxSource) error {
	...
	// verify tx source
	var verifyMods []TxVerifyMod
	if source == protocol.RPC {
		verifyMods = []TxVerifyMod{VerifyTxTime, VerifyTxInDB}
	} else if source == protocol.P2P {
		verifyMods = []TxVerifyMod{VerifyTxTime, VerifyTxAuth, VerifyTxInDB}
	} else {
	...
	if pool.queue.isFull(tx) {
		if source == protocol.RPC {
			_, err := pool.validateTx(tx, verifyMods...)
			if err != nil {
				return err
			}
	...
	mTx, err := pool.validateTx(tx, verifyMods...)
	if err != nil {
		return err
	}
...
func (pool *batchTxPool) dumpMsg(bz []byte, dir string) (err error) {
	// open wal
	var wal *lws.Lws
	if wal, err = lws.Open(dir); err != nil {
		return err
	}
	defer wal.Close()

	// write log entry
	if err = wal.Write(0, bz); err != nil {
		return err
	}
	// flush to file
	return wal.Flush()
