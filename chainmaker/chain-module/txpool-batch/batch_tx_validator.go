...
const (
	// valid endorsement num
	validEndorsementNum = 1
)
...
func (pool *batchTxPool) validateBatchSign(batch *txpoolPb.TxBatch) error {
	...
	// only verify whether principal is valid under PermissionedWithCert model
	if strings.ToLower(pool.chainConf.ChainConfig().AuthType) == protocol.PermissionedWithCert {
		...
		// only verify sign
		validEndorsements := pool.ac.RefineEndorsements(
			[]*commonPb.EndorsementEntry{batch.Endorsement},
			[]byte(batch.BatchId),
		)
		if len(validEndorsements) != validEndorsementNum {
			pool.log.Warnf("validateBatchSign verify endorsement failed")
			return fmt.Errorf("validateBatchSign verify endorsement failed")
		}
	}
