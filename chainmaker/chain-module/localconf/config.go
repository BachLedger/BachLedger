...
var (
	...
	// FindNewBlockChainNotifyC is the chan for finding new blockchain configs.
	FindNewBlockChainNotifyC = make(chan string)
	// FindDeleteBlockChainNotifyC is the chan for finding delete blockchain configs.
	FindDeleteBlockChainNotifyC = make(chan string)
	// ChainMakerConfig is the CMConfig instance for global.
	ChainMakerConfig = &CMConfig{StorageConfig: map[string]interface{}{}}
)

// newChainConfigWithDefault create a CMConfig instance with some default values
func newChainConfigWithDefault() *CMConfig {
	return &CMConfig{
...
func InitLocalConfig(cmd *cobra.Command) error {
	...
	// 2. deal and set the default configuration parameters
	cmConfig.Deal()

	// 3. set log config
	logger.SetLogConfig(&cmConfig.LogConfig)

	// 4. set global chainmaker config
...
func initCmConfig(cmd *cobra.Command) (*CMConfig, error) {
	...
	cmConfig := newChainConfigWithDefault() //&CMConfig{}
...
func CheckNewCmBlockChainConfig() error {
	...
	// 4. compare delete CMConfig with the current
	compareThenDeleteCMConfigWithCurrent(newCmConfig)
	return nil
}
...
func compareThenDeleteCMConfigWithCurrent(newCmConfig *CMConfig) {
	if len(ChainMakerConfig.BlockChainConfig) == 1 {
		return
	}
	// 4.1 load config file chain.
	configFileChain := make(map[string]struct{})
	for idx := range newCmConfig.BlockChainConfig {
		configFileChain[newCmConfig.BlockChainConfig[idx].ChainId] = struct{}{}
	}
	// 4.2 make sure old chains existed
	deleteChainIdsFound := make([]string, 0)
	for idx := range ChainMakerConfig.BlockChainConfig {
		oldChainId := ChainMakerConfig.BlockChainConfig[idx].ChainId
		_, ok := configFileChain[oldChainId]
		if !ok {
			deleteChainIdsFound = append(deleteChainIdsFound, oldChainId)
		}
	}
	if len(deleteChainIdsFound) > 0 {
		// 4.3 set new chain config
		if newCmConfig.NodeConfig.NodeId == "" {
			newCmConfig.SetNodeId(ChainMakerConfig.NodeConfig.NodeId)
		}
		ChainMakerConfig = newCmConfig
		// 4.4 send new block chain found notify
		for idx := range deleteChainIdsFound {
			FindDeleteBlockChainNotifyC <- deleteChainIdsFound[idx]
		}
	}
}
