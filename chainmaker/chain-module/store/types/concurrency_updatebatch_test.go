...
func TestInsert(t *testing.T) {
	m := New()
	elephant := Animal{elephantStr}
	monkey := Animal{monkeyStr}

	m.Set(elephantStr, elephant)
	m.Set(monkeyStr, monkey)
...
func TestInsertAbsent(t *testing.T) {
	...
	m.SetIfAbsent(elephantStr, elephant)
	if ok := m.SetIfAbsent(elephantStr, monkey); ok {
...
func TestGet(t *testing.T) {
	...
	if ok && elephant.name != elephantStr {
...
func TestRemoveCb(t *testing.T) {
	...
	cb := func(key string, val interface{}, exists bool) bool {
		...
		if animal, ok := val.(Animal); ok {
			return animal.name == monkeyStr
		}
		return false
	}

	// Monkey should be removed
	result := m.RemoveCb(monkeyStr, cb)
	if !result {
	...
	result = m.RemoveCb(elephantStr, cb)
	if result {
		t.Errorf("Result was true")
	}

	if mapKey != elephantStr {
		t.Error("Wrong key was provided to the callback")
	}
...
func TestIsEmpty(t *testing.T) {
	...
	m.Set(elephantStr, Animal{elephantStr})
...
func TestMInsert(t *testing.T) {
	animals := map[string]interface{}{
		elephantStr: Animal{elephantStr},
		monkeyStr:   Animal{monkeyStr},
	}
	m := New()
	m.MSet(animals)
