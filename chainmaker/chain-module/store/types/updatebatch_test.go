...
func TestUpdateBatch_IndexPairs(t *testing.T) {
	...
	v, err := batch.Get(key1)
	assert.Nil(t, err)
	assert.True(t, bytes.Equal(value1, v))
	v, err = batch.Get(key3)
	assert.Equal(t, errValueNotFound, err)
	assert.Nil(t, v)

	indexPairs := BatchToIndexPairs(batch.(*UpdateBatch))
	assert.Equal(t, batch.Len(), len(indexPairs))

	batchPool := NewSyncPoolUpdateBatch()
	batch2, ok := batchPool.Get().(*UpdateBatch)
	assert.True(t, ok)
	batch3 := IndexPairsToBatch(indexPairs, batch2, nil)
	assert.Equal(t, batch.Len(), batch3.Len())
	batch2.ReSet()
	assert.Equal(t, batch2.Len(), 0)
	batchPool.Put(batch2)
