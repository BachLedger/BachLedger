...
func TestHistorySqlDB_GetLastSavepoint(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, block1.Block.Header.BlockHeight, height)
	err = commitBlock(db, block2)
	assert.Nil(t, err)
	height, err = db.GetLastSavepoint()
	assert.Nil(t, err)
	assert.Equal(t, block2.Block.Header.BlockHeight, height)
}
...
//func TestResultFileDB_ShrinkBlocks(t *testing.T) {
//	db := initDb()
//	err := commitBlock(db, block1)
//	assert.Nil(t, err)
//	txMp := make(map[uint64][]string)
//	var txIds []string
//	for i := 0; i < len(block1.Block.Txs); i++ {
//		txIds = append(txIds, block1.Block.Txs[i].Payload.TxId)
//	}
//	txMp[block1.Block.Header.BlockHeight] = txIds
//	bfdbPath := path.Join(db.storeConfig.StorePath, testChainId, "bfdb")
//	shrinkErr := db.ShrinkBlocks(txMp, block1.Block.Header.BlockHeight, bfdbPath)
//	assert.Nil(t, shrinkErr)
//}

//func TestResultFileDB_ReWriteConfigBlockIndex(t *testing.T) {
//	db := initDb()
//	bufBytes, serializedBlock, serializeErr := serialization.SerializeBlock(block1)
//	assert.Nil(t, serializeErr)
//	serializedBlock.Index = &storePb.StoreInfo{
//		Offset:   0,
//		ByteLen:  uint64(len(bufBytes)),
//		FileName: fmt.Sprintf("%s%020d", "config_", 2),
//	}
//	writeErr := db.ReWriteConfigBlockIndex(serializedBlock)
//	assert.Nil(t, writeErr)
//	txId := block1.Block.Txs[0].Payload.TxId
//	rwSet, rwSetErr := db.GetRWSetIndex(txId)
//	assert.Nil(t, rwSetErr)
//	assert.NotNil(t, rwSet)
//}
