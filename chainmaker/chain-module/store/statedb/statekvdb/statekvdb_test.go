...
func TestStateKvDB_UselessFunc(t *testing.T) {
	db := initStateKvDB()
	_, err := db.QuerySingle("", "", nil)
	assert.NotNil(t, err)
	_, err = db.QueryMulti("", "", nil)
	assert.NotNil(t, err)
	_, err = db.BeginDbTransaction("")
	assert.NotNil(t, err)
	_, err = db.GetDbTransaction("")
	assert.NotNil(t, err)
	err = db.CommitDbTransaction("")
	assert.NotNil(t, err)
	err = db.RollbackDbTransaction("")
	assert.NotNil(t, err)
	err = db.ExecDdlSql("", "", "")
	assert.NotNil(t, err)
	...
	err = db.DropDatabase("")
	assert.NotNil(t, err)
	assert.Equal(t, db.GetContractDbName(""), "")
	db.Close()
}
