...
"EncryptKey":"1234567890123456"
	...
	var f = NewFactory()
	blkdb, errb := f.NewBlockDB("chainId", config, &test.GoLogger{})
	assert.Nil(t, errb)
	assert.NotNil(t, blkdb)
	blkdb.Close()

	bs, err := f.NewStore("chainId", config, &test.GoLogger{})
	assert.Nil(t, err)
	assert.NotNil(t, bs)
	t.Logf("%#v", bs)
	defer func() {
...
func TestReadEncryptKey(t *testing.T) {
	...
	t.Logf("path:%s", path)
	_ = ioutil.WriteFile(path, []byte(password), os.FileMode(0666))
