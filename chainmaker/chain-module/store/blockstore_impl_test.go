...
func createContractMgrPayload(txId string) *commonPb.Payload {
	p, _ := utils.GenerateInstallContractPayload(userContractName, "2.0",
		commonPb.RuntimeType_EVM, []byte("byte code!!!"), nil)
	p.TxId = txId
	return p
}
...
func testBlockchainStoreImpl_GetBlock(t *testing.T, config *conf.StorageConfig) {
	...
	s, err := factory.NewStore(chainId, config, &test.GoLogger{})
	if err != nil {
		panic(err)
	}
...
func Test_blockchainStoreImpl_PutBlock(t *testing.T) {
	...
	s, err := factory.NewStore(chainId, getSqliteConfig(), &test.GoLogger{})
	initGenesis(s)
...
func Test_blockchainStoreImpl_PutBlock2(t *testing.T) {
	...
	s, err := factory.NewStore(chainId, testConf, &test.GoLogger{}) //newStore(chainId, testConf)
	initGenesis(s)
	txRWSets := getTxRWSets()
	if err != nil {
		panic(err)
...
func Test_blockchainStoreImpl_PutBlock3(t *testing.T) {
	...
	s, err := factory.NewStore(chainId, configPutBlock3, &test.GoLogger{}) //factory.newStore(chainId, configPutBlock3)
	initGenesis(s)
	txRWSets := getTxRWSets()
	if err != nil {
		panic(err)
...
func Test_blockchainStoreImpl_PutBlock4(t *testing.T) {
	...
	s, err := factory.NewStore(chainId, configPutBlock4, &test.GoLogger{}) //factory.newStore(chainId, configPutBlock3)
	initGenesis(s)
	if err != nil {
		panic(err)
	}
...
func Test_blockchainStoreImpl_HasBlock(t *testing.T) {
	var factory = getFactory()
	s, err := factory.NewStore(chainId, getlvldbConfig(""), &test.GoLogger{}) //factory.newStore(chainId, getlvldbConfig(""))
	if err != nil {
		panic(err)
	}
	defer func() {
...
func Test_blockchainStoreImpl_TxExists(t *testing.T) {
	...
	var factory = getFactory()
	s, err := factory.NewStore(chainId, cfg, &test.GoLogger{})
	if err != nil {
		panic(err)
	}
	defer func(s protocol.BlockchainStore) {
...
func testSelectStateKeyRange(t *testing.T, storeConfig *conf.StorageConfig) {
	var factory = getFactory()
	s, err := factory.NewStore(chainId, storeConfig, &test.GoLogger{})
	defer func() { _ = s.Close() }()
...
func testKeyHistory(tt *testing.T, cf *conf.StorageConfig) {
	var factory = getFactory()
	s, err := factory.NewStore(chainId, cf, &test.GoLogger{})
	defer func() { _ = s.Close() }()
	init5ContractBlocks(s)
...
func TestBlockStoreImpl_GetTxRWSetsByHeight(t *testing.T) {
	var factory = getFactory()
	s, err := factory.NewStore(chainId, getlvldbConfig(""), &test.GoLogger{})
	defer func() { _ = s.Close() }()
	init5Blocks(s)
	txRWSets := getTxRWSets()
	assert.Equal(t, nil, err)
...
func Test_blockchainStoreImpl_recovory(t *testing.T) {
	...
	s, err := factory.NewStore(chainId, ldbConfig, &test.GoLogger{})
	//defer s.Close()
	assert.Equal(t, nil, err)
...
func Test_blockchainStoreImpl_GetBlockWithTimeout(t *testing.T) {
	...
	s, err := factory.NewStore(chainId, lvlConf, &test.GoLogger{})
	if err != nil {
		panic(err)
	}
	defer func() { _ = s.Close() }()
...
func TestBlockStoreImpl_GetBlockHeaderByHeight(t *testing.T) {
	...
	s, err := factory.NewStore(chainId, sqldbConfig, &test.GoLogger{})
	assert.Equal(t, nil, err)
	bs, ok := s.(*BlockStoreImpl)
	assert.Equal(t, true, ok)
	init5Blocks(s)
...
func TestBlockStoreImpl_GetTxHeight(t *testing.T) {
	...
	s, err := factory.NewStore(chainId, lvldbConfig, &test.GoLogger{})
	assert.Equal(t, nil, err)
	bs, ok := s.(*BlockStoreImpl)
	assert.Equal(t, true, ok)
	//init5Blocks(s)
...
func TestSlowLog(t *testing.T) {
	...
	bdb.EXPECT().GetArchivedPivot().Return(uint64(0), nil)

	lvldbConfig := getlvldbConfig("")
	lvldbConfig.SlowLog = 100
	s, err := NewBlockStoreImpl("c1", lvldbConfig, bdb, sdb, nil,
