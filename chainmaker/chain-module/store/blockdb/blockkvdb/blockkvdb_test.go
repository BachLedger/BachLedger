...
func initDb() *BlockKvDB {
	blockDB := NewBlockKvDB("test-chain", leveldbprovider.NewMemdbHandle(), logger, config1)
	return blockDB
}
...
func TestBlockKvDB_GetArchivedPivot(t *testing.T) {
	...
	err = db.dbHandle.Put([]byte(blockhelper.ArchivedPivotKey), blockhelper.ConstructBlockNumKey(db.dbHandle.GetDbType(), 10))
	assert.Nil(t, err)
	archivedPivot, err = db.GetArchivedPivot()
...
//	}
//
//	func BenchmarkBlockKvDB_constructTxIDBlockInfo(b *testing.B) {
//		hash := sha256.Sum256([]byte("hello"))
//		for i := 0; i < b.N; i++ {
//			constructTxIDBlockInfo(uint64(i), hash[:], uint32(i), int64(i))
//		}
//	}
func BenchmarkBlockKvDB_TxInfo(b *testing.B) {
	hash := sha256.Sum256([]byte("hello"))
	for i := 0; i < b.N; i++ {
...
//func TestBlockKvDB(t *testing.T) {
