...
// NewDPoSStakeContract comment at next version
// @param log
...
func registerDPoSStakeContractMethods(log protocol.Logger) map[string]common.ContractFunc {
	...
	methodMap[syscontract.DPoSStakeFunction_GET_ALL_CANDIDATES.String()] = common.WrapResultFunc(
		stakeRuntime.GetAllCandidates)
	methodMap[syscontract.DPoSStakeFunction_GET_VALIDATOR_BY_ADDRESS.String()] = common.WrapResultFunc(
		stakeRuntime.GetValidatorByAddress)
	methodMap[syscontract.DPoSStakeFunction_DELEGATE.String()] = common.WrapResultFunc(
		stakeRuntime.Delegate)
	methodMap[syscontract.DPoSStakeFunction_GET_DELEGATIONS_BY_ADDRESS.String()] = common.WrapResultFunc(
		stakeRuntime.GetDelegationsByAddress)
	methodMap[syscontract.DPoSStakeFunction_GET_USER_DELEGATION_BY_VALIDATOR.String()] = common.WrapResultFunc(
		stakeRuntime.GetUserDelegationByValidator)
	methodMap[syscontract.DPoSStakeFunction_UNDELEGATE.String()] = common.WrapResultFunc(
		stakeRuntime.UnDelegate)
	methodMap[syscontract.DPoSStakeFunction_READ_EPOCH_BY_ID.String()] = common.WrapResultFunc(
		stakeRuntime.ReadEpochByID)
	methodMap[syscontract.DPoSStakeFunction_READ_LATEST_EPOCH.String()] = common.WrapResultFunc(
		stakeRuntime.ReadLatestEpoch)
	methodMap[syscontract.DPoSStakeFunction_SET_NODE_ID.String()] = common.WrapResultFunc(
		stakeRuntime.SetNodeID)
	methodMap[syscontract.DPoSStakeFunction_GET_NODE_ID.String()] = common.WrapResultFunc(
		stakeRuntime.GetNodeID)
	methodMap[syscontract.DPoSStakeFunction_READ_MIN_SELF_DELEGATION.String()] = common.WrapResultFunc(
		stakeRuntime.ReadMinSelfDelegation)
	//methodMap[syscontract.DPoSStakeFunction_UPDATE_MIN_SELF_DELEGATION.String()] = common.WrapResultFunc(
	//DPoSStakeRuntime.UpdateMinSelfDelegation)
	methodMap[syscontract.DPoSStakeFunction_READ_EPOCH_VALIDATOR_NUMBER.String()] = common.WrapResultFunc(
		stakeRuntime.ReadEpochValidatorNumber)
	//methodMap[syscontract.DPoSStakeFunction_UPDATE_EPOCH_VALIDATOR_NUMBER.String()] = common.WrapResultFunc(
	//DPoSStakeRuntime.UpdateEpochValidatorNumber)
	methodMap[syscontract.DPoSStakeFunction_READ_EPOCH_BLOCK_NUMBER.String()] = common.WrapResultFunc(
		stakeRuntime.ReadEpochBlockNumber)
	methodMap[syscontract.DPoSStakeFunction_READ_SYSTEM_CONTRACT_ADDR.String()] = common.WrapResultFunc(
		stakeRuntime.ReadSystemContractAddr)
	//methodMap[syscontract.DPoSStakeFunction_UPDATE_EPOCH_BLOCK_NUMBER.String()] = common.WrapResultFunc(
	//DPoSStakeRuntime.UpdateEpochBlockNumber)
	methodMap[syscontract.DPoSStakeFunction_READ_COMPLETE_UNBOUNDING_EPOCH_NUMBER.String()] = common.WrapResultFunc(
...
// GetAllCandidates 返回所有满足最低抵押条件验证人候选人
...
// GetValidatorByAddress 返回所有满足最低抵押条件验证人
// @params["address"]
// return Validator
...
// Delegate 创建抵押，更新验证人，如果MsgSender是给自己，即给自己抵押，则创建验证人
// @params["to"] 		抵押的目标验证人
...
// GetDelegationsByAddress 返回所有 Delegation
// @params["address"]
...
// GetUserDelegationByValidator 返回所有 Delegation
...
func (s *DPoSStakeRuntime) canDelete(context protocol.TxSimContext, undelegateValidatorAddress string) error {
	...
	if len(bz) <= 0 {
		return fmt.Errorf(fmt.Sprintf(ErrorGetValueByKey, "KeyEpochValidatorNumber"))
	}
...
func (s *DPoSStakeRuntime) ReadLatestEpoch(context protocol.TxSimContext, params map[string][]byte) ([]byte, error) {
	...
	if len(bz) <= 0 {
		return nil, fmt.Errorf(fmt.Sprintf(ErrorGetValueByKey, "KeyCurrentEpoch"))
	}
	return bz, nil
}

// ReadEpochByID 读取指定ID的世代数据
...
// ReadMinSelfDelegation  读取验证人最少抵押token数量
// return string
func (s *DPoSStakeRuntime) ReadMinSelfDelegation(
	...
	if len(bz) <= 0 {
		return nil, fmt.Errorf(fmt.Sprintf(ErrorGetValueByKey, "KeyMinSelfDelegation"))
	}
	return bz, nil
}

// UpdateMinSelfDelegation 更新验证人最少抵押token数量，当提高最少抵押门槛时，原先的 validator 状态不会更新，validator 需要尽块增加抵押
...
// ReadEpochValidatorNumber 读取每个世代验证人数量
// return string
func (s *DPoSStakeRuntime) ReadEpochValidatorNumber(
	...
	if len(bz) <= 0 {
		return nil, fmt.Errorf(fmt.Sprintf(ErrorGetValueByKey, "KeyEpochValidatorNumber"))
	}
	amount := decodeUint64FromBigEndian(bz)
	return []byte(strconv.Itoa(int(amount))), nil
}

// UpdateEpochValidatorNumber 更新每个世代验证人数量，不能大于当前所有验证人数量
...
// ReadEpochBlockNumber  读取世代的出块数量
// return string
func (s *DPoSStakeRuntime) ReadEpochBlockNumber(
	...
	if len(bz) <= 0 {
		return nil, fmt.Errorf(fmt.Sprintf(ErrorGetValueByKey, "KeyEpochBlockNumber"))
	}
	amount := decodeUint64FromBigEndian(bz)
	return []byte(strconv.Itoa(int(amount))), nil
}

// ReadSystemContractAddr  读取stake系统合约的地址
// return string
...
// ReadCompleteUnBoundingEpochNumber 读取世代的出块数量
// return string
...
// UpdateEpochBlockNumber  更新世代的出块数量
...
func getOrCreateValidator(
	...
	if delegatorAddress != validatorAddress {
		...
		return nil, fmt.Errorf("no such validator: %s", validatorAddress)
	}
...
func getDelegationsByAddress(
	...
	// 获取地址所有抵押数据
	start, limit := bytehelper.BytesPrefix(ToDelegationPrefix(delegatorAddress))
	// TODO search scope has no memory data
	iter, err := context.Select(syscontract.SystemContract_DPOS_STAKE.String(), start, limit)
	if err != nil {
		return nil, err
	}
	defer iter.Release()
...
func getAllValidatorByPrefix(context protocol.TxSimContext, prefix []byte) ([]*syscontract.Validator, error) {
	// 获取所有验证人数据
	start, limit := bytehelper.BytesPrefix(prefix)
	iter, err := context.Select(syscontract.SystemContract_DPOS_STAKE.String(), start, limit)
	if err != nil {
		return nil, err
	}
...
func getUnbondingEpochNumber(context protocol.TxSimContext) (uint64, error) {
	...
	if len(bz) <= 0 {
		return 0, fmt.Errorf(fmt.Sprintf(ErrorGetValueByKey, "KeyCompletionUnbondingEpochNumber"))
	}
