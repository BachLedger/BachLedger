...
func initTestEnv(t *testing.T) (*PubkeyManageRuntime, protocol.TxSimContext, func()) {
	...
	cache := NewCacheMock()
	mockChainConfig := &configPb.ChainConfig{Version: "2300"}
	data, _ := mockChainConfig.Marshal()
	cache.Put(syscontract.SystemContract_CHAIN_CONFIG.String(), "CHAIN_CONFIG", data)
...
// TestAddPubkey comment at next version
func TestAddPubkey(t *testing.T) {
	...
	result, _, err := pmRuntime.AddPubkey(context, params)
	if err != nil {
...
// TestDelPubkey comment at next version
func TestDelPubkey(t *testing.T) {
	...
	delparams["org_id"] = []byte("org1")
	delparams["pubkey"] = []byte(strPubkey)

	result2, _, err := pmRuntime.DeletePubkey(context, delparams)
...
// TestQueryPubkey comment at next version
