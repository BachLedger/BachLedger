...
// NewDPoSERC20Contract comment at next version
// @param log
...
func registerDPoSERC20ContractMethods(log protocol.Logger) map[string]common.ContractFunc {
	...
	methodMap[syscontract.DPoSERC20Function_GET_BALANCEOF.String()] = common210.WrapResultFunc(dposRuntime.BalanceOf)
	methodMap[syscontract.DPoSERC20Function_TRANSFER.String()] = common210.WrapResultFunc(dposRuntime.Transfer)
	//methodMap[syscontract.DPoSERC20Function_TRANSFER_FROM.String()] =
	//common210.WrapResultFunc(  dposRuntime.TransferFrom)
	//methodMap[syscontract.DPoSERC20Function_GET_ALLOWANCE.String()] = common210.WrapResultFunc(  dposRuntime.Allowance)
	//methodMap[syscontract.DPoSERC20Function_APPROVE.String()] = common210.WrapResultFunc(  dposRuntime.Approve)
	methodMap[syscontract.DPoSERC20Function_MINT.String()] = common210.WrapResultFunc(dposRuntime.Mint)
	//methodMap[syscontract.DPoSERC20Function_BURN.String()] = common210.WrapResultFunc(  dposRuntime.Burn)
	//methodMap[syscontract.DPoSERC20Function_TRANSFER_OWNERSHIP.String()] = common210.WrapResultFunc(
	//dposRuntime.TransferOwnership)
	methodMap[syscontract.DPoSERC20Function_GET_OWNER.String()] = common210.WrapResultFunc(dposRuntime.Owner)
	methodMap[syscontract.DPoSERC20Function_GET_DECIMALS.String()] = common210.WrapResultFunc(dposRuntime.Decimals)
	methodMap[syscontract.DPoSERC20Function_GET_TOTAL_SUPPLY.String()] = common210.WrapResultFunc(dposRuntime.Total)
	return methodMap
}
...
// Transfer It is equal with transfer(to string, value uint64) for ETH
// params["to"]:${to}
// params["value"]:${value}
...
// Approve It is equal with approve(spender string, value uint64) error; for ETH
// params["to"]:${to}
// params["value"]:${value}
...
func (r *DPoSRuntime) Burn(txSimContext protocol.TxSimContext, params map[string][]byte) (result []byte, err error) {
	...
	if beforeFromBalance == nil {
		...
		return nil, fmt.Errorf(
			"load address balance error which is zero, contract[%s] address[%s]",
			dposErc20ContractName, from)
	}
	// 处理总量
	...
	if beforeTotalSupply.Cmp(val) < 0 {
		r.log.Errorf(
			"total supply is not enough for burn, before[%s] burn-value[%s]", beforeTotalSupply.String(), val.String())
		return nil, fmt.Errorf(
			"total supply is not enough for burn, before[%s] burn-value[%s]", beforeTotalSupply.String(), val.String())
	}
	// 计算燃烧后的值
	...
	if beforeFromBalance.Cmp(val) < 0 {
		r.log.Errorf(
			"address[%s] is not enough for burn, before[%s] burn-value[%s]",
			from, beforeTotalSupply.String(), val.String())
		return nil, fmt.Errorf(
			"address[%s] is not enough for burn, before[%s] burn-value[%s]",
			from, beforeTotalSupply.String(), val.String())
	}
...
// Allowance comment at next version
// params["from"]:${from}
// params["to"]:${to}
...
func loadSenderAddress(txSimContext protocol.TxSimContext) (string, error) {
	...
	case accesscontrol.MemberType_CERT:
		// 长证书
		if pubkey, err = getPubkeyFromCert(sender.MemberInfo); err != nil {
			return "", err
		}
	case accesscontrol.MemberType_CERT_HASH:
		...
		if pubkey, err = getPubkeyFromCert(certInfo.Cert); err != nil {
			return "", err
		}
	...
	return calUserAddr(pubkey, ac.GetHashAlg())
}

// parseUserAddress
func getPubkeyFromCert(member []byte) ([]byte, error) {
	certificate, err := utils.ParseCert(member)
	if err != nil {
		return nil, fmt.Errorf("parse cert failed, name[%s] err: %+v", dposErc20ContractName, err)
	}
	pubKeyBytes, err := certificate.PublicKey.Bytes()
	if err != nil {
		return nil, fmt.Errorf("load public key from cert failed, name[%s] err: %+v", dposErc20ContractName, err)
	}
...
func transfer(txSimContext protocol.TxSimContext, from, to string, val *utils.BigInteger) ([]byte, error) {
	...
	if fromBalance == nil {
		return nil, fmt.Errorf(
			"load from address balance error which is zero, contract[%s] address[%s]",
			dposErc20ContractName, from)
	}
	// 判断其值是否满足
	if fromBalance.Cmp(val) < 0 {
		// 账户剩余的钱不满足需求
		return nil, fmt.Errorf(
			"address balance is not enough, contract[%s] from address[%s] balance[%s] < value[%s]",
	...
	if beforeSum.Cmp(afterSum) != 0 {
		// 前后值存在问题
		return nil, fmt.Errorf(
			"balance is not equal before and after operation, contract[%s] before-balance[%s] after-balance[%s]",
