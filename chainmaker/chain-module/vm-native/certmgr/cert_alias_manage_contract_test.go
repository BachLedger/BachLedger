...
func Test_AddAlias(t *testing.T) {
	...
	params[paramNameAlias] = []byte("alias.789-123456789_12345678901234567890123456789012345678901234")
	...
	// bad param 65
	params[paramNameAlias] = []byte("alias678901234567890123456789012345678901234567890123456789012345")
	...
	// bad param
	params[paramNameAlias] = []byte("_alias01")
	_, err = mgrRuntime.AddAlias(txSimContext, params)
	assert.NotNil(t, err)
...
func Test_UpdateAlias(t *testing.T) {
	...
	// no params
	result, event, err := mgrRuntime.UpdateAlias(txSimContext, nil)
	assert.NotNil(t, err)
	assert.Nil(t, result)
	assert.Nil(t, event)
	...
	params2[paramNameAlias] = aliasName2
...
func Test_DeleteAlias(t *testing.T) {
	...
	// no params
	result, event, err := mgrRuntime.DeleteAlias(txSimContext, nil)
	assert.NotNil(t, err)
	assert.Nil(t, result)
	assert.Nil(t, event)
	...
	// no alias add
	params := make(map[string][]byte)
	aliasName3 := []byte("alias03")
	params[paramNameAliases] = aliasName3
	result, event, err = mgrRuntime.DeleteAlias(txSimContext, params)
	assert.NotNil(t, err)
	assert.Nil(t, result)
	assert.Nil(t, event)
	...
	// add alias
	params2 := make(map[string][]byte)
	params2[paramNameAlias] = aliasName3
	result2, err2 := mgrRuntime.AddAlias(txSimContext, params2)
	assert.Nil(t, err2)
	assert.Equal(t, "ok", string(result2)) // 必须，否则需兼容版本
	...
	params[paramNameAliases] = aliasName3
	params[paramNameCert] = getOrg2Client1Signer().MemberInfo
	result, event, err = mgrRuntime2.DeleteAlias(txSimContext2, params)
	assert.NotNil(t, err)
	assert.Nil(t, result)
	assert.Nil(t, event)
