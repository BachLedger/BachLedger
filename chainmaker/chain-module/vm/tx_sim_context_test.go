...
func Test_txSimContextImpl_CallContract(t *testing.T) {
	...
	ac.EXPECT().LookUpExceptionalPolicy(gomock.Any()).Return(&pbac.Policy{}, nil).AnyTimes()
	...
	ac.EXPECT().LookUpPolicy(gomock.Any()).Return(&pbac.Policy{}, errors.New("error")).AnyTimes()
	...
	accessControlProvider.EXPECT().LookUpPolicy(gomock.Any()).Return(
		&acPb.Policy{
			Rule: string(protocol.RuleSelf),
		}, nil).AnyTimes()
	accessControlProvider.EXPECT().LookUpExceptionalPolicy(gomock.Any()).Return(
		&acPb.Policy{}, nil).AnyTimes()
	accessControlProvider.EXPECT().CreatePrincipal(gomock.Any(), gomock.Any(), gomock.Any()).Return(
		&mock.MockPrincipal{}, nil).AnyTimes()
	accessControlProvider.EXPECT().VerifyPrincipal(gomock.Any()).Return(true, nil).AnyTimes()
	...
	vmManager.EXPECT().GetAccessControl().Return(accessControlProvider).AnyTimes()
	type args struct {
		contract  *common.Contract
		method    string
		byteCode  []byte
