...
func (w *WacsiWithGasImpl) GetState(
	...
	if err := gaswasm.SubtractGasForGetState(contractName, stateKey, value, txSimContext); err != nil {
		return nil, err
	}
...
func (w *WacsiWithGasImpl) CallContract(
	...
	if err := gaswasm.SubtractGasForCallContract(
		contractName, method, paramMap, result.Result, txSimContext); err != nil {
		return nil, gasUsed, protocol.ExecOrderTxTypeNormal, err
	}
...
func (w *WacsiWithGasImpl) EmitEvent(
	...
	if err := gaswasm.SubtractGasForEmitEvent(topic, eventData, txSimContext); err != nil {
		return nil, err
	}
...
func (w *WacsiWithGasImpl) KvPreIterator(requestBody []byte, contractName string, txSimContext protocol.TxSimContext,
	...
	if err := gaswasm.SubtractGasForKvIterator([]byte(key), []byte(limit), txSimContext); err != nil {
		return err
	}
...
func (w *WacsiWithGasImpl) ExecuteQueryOne(requestBody []byte, contractName string, txSimContext protocol.TxSimContext,
	...
	if isLen {
		...
		rsBytes := ec.Marshal()
		if err := gaswasm.SubtractGasForExecuteQueryOne(sql, rsBytes, txSimContext); err != nil {
			return nil, err
		}
...
func (w *WacsiWithGasImpl) RSNext(requestBody []byte, txSimContext protocol.TxSimContext, memory []byte, data []byte,
	...
	if isLen {
		...
		ec := serialize.NewEasyCodecWithMap(dataRow)
		rsBytes := ec.Marshal()
		if err := gaswasm.SubtractGasForRSNext(rsIndex, rsBytes, txSimContext); err != nil {
			return nil, err
		}
