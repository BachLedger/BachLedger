...
func (m *VmManagerImpl) runNativeContract(contract *commonPb.Contract, method string, parameters map[string][]byte,
	...
	// store runtime type from parameters, prevent modification of parameters
	runtimeTypeParam := string(parameters[syscontract.InitContract_CONTRACT_RUNTIME_TYPE.String()])
	...
	if runtimeContractResult.Code == uint32(0) {
		m.saveNativeContractMetrics(contract.Name, method, runtimeTypeParam, "true")
	...
	m.saveNativeContractMetrics(contract.Name, method, runtimeTypeParam, "false")
	if contract.Name == syscontract.SystemContract_ACCOUNT_MANAGER.String() &&
		method == syscontract.GasAccountFunction_CHARGE_GAS_FOR_MULTI_ACCOUNT.String() {
...
func (m *VmManagerImpl) invokeUserContractByRuntime(contract *commonPb.Contract, method string,
	...
	*commonPb.ContractResult, protocol.ExecOrderTxType, commonPb.TxStatusCode) {
	...
	m.Log.Infof("core loop: runtype[%v], %v", runtimeType, m.InstanceManagers)
	...
	m.Log.Infof("core loop: invoke vm, tx id:%s, tx type:%+v, contractId:%+v, method:%+v,"+
	...
	if runtimeContractResult.Code == 0 {
		m.saveUserContractMetrics(txType, contract, runtimeType, 0, "true")
	...
	m.saveUserContractMetrics(txType, contract, runtimeType, 0, "false")
...
func (m *VmManagerImpl) saveNativeContractMetrics(contractName, method, runtimeType, state string) {
	if !localconf.ChainMakerConfig.MonitorConfig.Enabled {
		return
	}
	...
	if contractName == syscontract.SystemContract_CONTRACT_MANAGE.String() &&
		...
		m.metricDeployedContractCounter.WithLabelValues(m.ChainId, runtimeType, state).Inc()
	}
}
