...
func (s *txSimContextImpl) CallContract(caller, contract *common.Contract, method string, byteCode []byte,
	...
	*common.ContractResult, protocol.ExecOrderTxType, common.TxStatusCode) {
	...
	if len(byteCode) == 0 && contract.RuntimeType != common.RuntimeType_GO {
	...
	blockVersionStr := strconv.FormatUint(uint64(s.blockVersion), 10)
	if err := s.verifyCallContract(contract, method, blockVersionStr); err != nil {
...
func (s *txSimContextImpl) verifyCallContract(contract *common.Contract, method string, options ...string) error {

	s.logger.Debugf("origin contract name = %v", s.tx.Payload.ContractName)
	s.logger.Debugf("origin method = %v", s.tx.Payload.Method)

	return s.verifyPolicy(contract, method, options...)
}

func (s *txSimContextImpl) verifyPolicy(contract *common.Contract, method string, options ...string) error {
	...
	endorsements := []*common.EndorsementEntry{tx.Sender}
	txType := tx.Payload.TxType
	resourceId := contract.Name + "-" + method
	...
	if tx.Payload.TxType == common.TxType_INVOKE_CONTRACT {
		p, err := ac.LookUpPolicy(resourceId)
		s.logger.Debugf("ac.LookUpPolicy(), policy = %v", p)
		if err != nil {
			return nil
		}
			...
			if parameterPairs != nil {
				...
				if err != nil {
					return fmt.Errorf("fail to construct authentication principal with orgId %s for %s-%s: %s",
						targetOrg, tx.Payload.ContractName, tx.Payload.Method, err)
				}
			}
		} else {
			principal, err = ac.CreatePrincipal(resourceId, endorsements, txBytes)
		...
		ok, err := ac.VerifyPrincipal(principal)
		if err != nil {
			return fmt.Errorf("authentication error for %s-%s: %s", tx.Payload.ContractName, tx.Payload.Method, err)
		}
		if !ok {
			return fmt.Errorf("authentication failed for %s-%s", tx.Payload.ContractName, tx.Payload.Method)
		}
	}
	return nil
}
